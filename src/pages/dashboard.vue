        ...
        <a-row :gutter="[16, 16]">
          <a-col :span="24">
            <div class="sound_div">
              ...
              <span class="sound_text" v-else> 暂无告警信息 </span>
            </div>
          </a-col>
          <a-col :span="6">
            ...
            <a-card size="small">
              <span slot="title" class="titlex">
                ...
                <div class="dateSelect">
                  <span :class="`cur ` + execType1" @click="onLoadExec_cnt(1)"
                    >今日</span
                  >
                  <a-divider type="vertical" />
                  <span :class="`cur ` + execType2" @click="onLoadExec_cnt(2)"
                    >昨日</span
                  >
                  <a-divider type="vertical" />
                  <span :class="`cur ` + execType3" @click="onLoadExec_cnt(3)"
                    >本周</span
                  >
                  <a-divider type="vertical" />
                  <span :class="`cur ` + execType4" @click="onLoadExec_cnt(4)"
                    >本月</span
                  >
                  <a-divider type="vertical" />
                  <span :class="`cur ` + execType5" @click="onLoadExec_cnt(5)"
                    >上月</span
                  >
                  <a-divider type="vertical" />
                  <span :class="`cur ` + execType6" @click="onLoadExec_cnt(6)"
                    >本年</span
                  >
                </div>
              </span>
              ...
              >
                ...
                >
                  <a-avatar
                    :size="22"
                    style="margin-right: 3px"
                    :src="BaseURL + '/app/basic/unknown.png'"
                  />
                  <a-tag color="#53b0b5">{{ text }} </a-tag>
                </span>
        ...
        <a-row :gutter="[16, 16]">
          <a-col :span="24">
            <div
              ...
              style="font-size: 14px; color: #dbdbdb"
            >
...
export default {
  ...
  methods: {
    ...
    removeNotification(index) {
      ...
      const alertId = notification.alertInfo
        ? notification.alertInfo.alert_id
        : null;
    ...
    renderNotificationModal(item, data) {
      ...
      // 获取analysis数据中的第一条记录
      const analysisData =
        analysis && analysis.list && analysis.list.length > 0
          ? analysis.list[0]
          : null;
              ...
              ? h("div", { style: { marginBottom: "20px" } }, [
                  ...
                  h(
                    ...
                    [
                      ...
                      (decision && decision.natural_language_decision) ||
    ...
    renderFlowchart(recommendedActions) {
      ...
      lines.forEach((line) => {
        ...
        if (trimmedLine.includes("->")) {
          ...
          const workflowText = isOptimal
            ? trimmedLine.replace("推荐最优解：", "").trim()
            : trimmedLine;
        ...
        workflows.map((workflow, workflowIndex) => {
          ...
          return h(
            "div",
            {
              ...
              style: {
                marginBottom:
                  workflowIndex < workflows.length - 1 ? "16px" : "0",
                padding: "12px",
                border: workflow.isOptimal
                  ? "2px solid #52c41a"
                  : "1px solid #d9d9d9",
                borderRadius: "8px",
